...
<div class="popup-box chat-popup" id="qnimate">
    ...
    <div class="popup-messages" id="popup-messages">
        <div class="direct-chat-messages" id="conversation-list" style="overflow: auto">
            @php
                $conversation = \App\Models\Conversation::where('user_sender_id', \Auth::user()->id)->where('status', '<>', \App\Models\Conversation::DONE)->first();
                if($conversation) {
            ...
            @if(isset($messageList))
                ...
                @endforeach
                @else
                <p class="text-center" style="color: white"> Start Conversation with Admin</p>
                <hr>
            @endif
        </div>
    </div>
